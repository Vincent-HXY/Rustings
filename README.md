...
* 在线环境（云原生开发）：
  ...
  1. 在仓库页面的分支列表中，点击 main 分支旁的「云原生开发」按钮，即可启动在线开发环境（WebIDE），自动配置好 Rust 工具链和 rustlings。

  2. 在 WebIDE 的终端中执行 `rustlings watch`，即可开始练习。

  3. 完成练习后，在终端中执行 `git add .; git commit -m "update"; git push` 提交代码，平台会自动评测并上传分数。

3. 上述步骤有任何问题都可以找助教。

4. 下面是官方的Rustlings的说明，可以参考，**请务必不要拉取下面的仓库！**
